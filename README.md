...
1. [translation\_stats\_by\_sel\_lang\_and\_ver.sparql](translation\_stats\_by\_sel\_lang\_and\_ver.sparql): we think this gives the translation counts for selected
languages presented on the dashboard in the 'Number of Translations' figures on
the General tab (which presents the data by Wiktionary) and on the tabs for the
...
2. [translation\_counts\_by\_lang\_in\_en.sparql](translation\_counts\_by\_lang\_in\_en.sparql): This contains three example queries:
   (a) translation counts for all target languages from the current English-language Wiktionary;
   (b) translation counts for selected languages from the same Wiktionary;
   (c) translation counts for selected languages with output labels manually assigned from the same Wiktionary
...
The repository [wiki](https://github.com/ghrgriner/dbnary-utils/wiki)
